...
Component.entryPoint = function(NS){
	...
	ImageListWidget.prototype = {
		...
		onClick: function(el){
			...
			for (var i=0;i<tabs.length;i++){
				if (tabs[i].onClick(el)){ return true; }
			}
			return false;
		},
		toSave: function(){
	...
	YAHOO.extend(DrawTab, YAHOO.widget.Tab, {
		...
		onClick: function(el){
			...
			switch(el.id){
			case tp['bimgupload']: this.imageUpload(); return true;
			case tp['bshowfm']: this.showFileBrowser(); return true;
			case tp['bimgedit']: this.editTitle(); return true;
			case tp['btabremove']: this.removeThisTab(); return true;
			}
	...
	DrawWidget.prototype = {
		...
		onClick: function(el){
			...
			case tp['clrpicker']:
				...
				return true;
			case tp['tlhand']: this.selectTool('hand'); return true;
			case tp['tlpath']: this.selectTool('path'); return true;
			case tp['tlerase']: this.selectTool('erase'); return true;
			case tp['tlcomment']: this.selectTool('cmt'); return true;
			case tp['tlzoomin']: return this.imageZoomIn();
			}
			return false;
		},
	...
	YAHOO.extend(CommentFeature, Feature, {
		...
		onClick: function(el){
			...
			switch(el.id){
			case tp['bclose']: this.remove(); return true;
	...
	YAHOO.extend(EraseTool, DrawTool, {
		...
		onClick: function(e){
			if (L.isNull(this._curFeature)){ return; }
	...
	YAHOO.extend(CommentTool, DrawTool, {
		...
		startDraw: function(e){
			if (!L.isNull(this.gline)){ this.stopDraw(e); }
			...
			this.gline = gline;
			var p = this.path = [0,0,0,0]; p[0] = e.x; p[1] = e.y;
		},
		moveDraw: function(e){
			if (L.isNull(this.gline)){ return; }
			
			var p = this.path; p[2] = e.x; p[3] = e.y;
	...
	DrawToolList.prototype = {
		...
		getByIndex: function(index){ return this._list[index]; }
	};
	...
	DrawToolManager.prototype = {
		...
		mouseEvent: function(evt){
			...
			if (L.isNull(layer)){ return; }
			var xy = this._getXY(evt),
				x = xy[0], y = xy[1];
			var tool = this.selected;
			if (!L.isFunction(tool[EFN[evt.type]])){ return; }
	...
	Canvas.prototype = {
		...
		_initCanvas: function(container, config){
			...
			E.on(el, 'mousedown',	function(evt){ __self._mouseEvent(evt); });
			E.on(el, 'mouseup',		function(evt){ __self._mouseEvent(evt); });
			E.on(el, 'mousemove',	function(evt){ __self._mouseEvent(evt); });
			E.on(el, 'mouseover',	function(evt){ __self._mouseEvent(evt); });
			E.on(el, 'mouseout',	function(evt){ __self._mouseEvent(evt); });
			E.on(el, 'click',		function(evt){ __self._mouseEvent(evt); });
